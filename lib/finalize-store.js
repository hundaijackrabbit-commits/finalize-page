...
const STORAGE_KEY = 'finalize.phase5.v1';
...
export function completionSignals(finalization) {
  ...
  for (const b of finalization.integrationBindings || []) if (b.required && !b.requirementId && b.status !== 'satisfied') signals.push(synthetic(`integration:${b.id}`, `${b.signalKey} must reach ${b.expectedState}`, 'External evidence', 'integration'));
  ...
  for (const artifact of finalization.artifacts || []) for (const f of artifact.documentFindings || []) if (f.severity === 'BLOCKER' && f.status === 'OPEN' && (f.artifactVersion == null || Number(f.artifactVersion) === Number(finalization.artifactVersion))) signals.push({ ...synthetic(`document:${f.id}`, f.title, 'Document', 'document'), artifactId: artifact.id, findingId: f.id, evidence: f.evidence });
  return signals;
}
...
export function readiness(finalization) {
  ...
  const documentBlockerTotal = (finalization.artifacts || []).reduce((n,a) => n + (a.documentFindings || []).filter((f) => f.severity === 'BLOCKER' && f.status === 'OPEN' && (f.artifactVersion == null || Number(f.artifactVersion) === Number(finalization.artifactVersion))).length, 0);
  const externalTotal = (finalization.fileRequests || []).filter((r) => r.required).length + (finalization.secureRequests || []).length + (finalization.paymentGates || []).length + (finalization.privacyItems || []).filter((i) => i.required).length + (finalization.annotations || []).filter((a) => a.artifactVersion === finalization.artifactVersion).length + documentBlockerTotal;
  ...
  const documentWarnings = (finalization.artifacts || []).flatMap((a) => (a.documentFindings || []).filter((f) => f.severity === 'WARNING' && f.status === 'OPEN' && (f.artifactVersion == null || Number(f.artifactVersion) === Number(finalization.artifactVersion))).map((f) => ({...f, artifactId:a.id, source:'document'})));
  const warnings = [...recommended.filter((r) => !['passed','waived'].includes(r.status)), ...documentWarnings];
...
export function useFinalizeStore() {
  ...
  const remoteDocumentAction = useCallback(async (id, action, payload = {}) => {
    const response = await fetch(`/api/finalizations/${id}/documents`, { method:'POST', headers:{'content-type':'application/json'}, body:JSON.stringify({action,...payload}) });
    const data = await response.json();
    if (!response.ok) throw new Error(data.detail || data.error || 'Document action failed');
    if (data.finalization) replaceOne(data.finalization);
    return data;
  }, [replaceOne]);
  ...
  const setDocumentFindingStatus = useCallback(async(finalizationId,findingId,status,note='')=>{ if(REMOTE_MODE)return remoteDocumentAction(finalizationId,'set_finding_status',{findingId,status,note}); mutate(finalizationId,(f)=>({...f,artifacts:(f.artifacts||[]).map((a)=>({...a,documentFindings:(a.documentFindings||[]).map((x)=>x.id===findingId?{...x,status,resolutionNote:note,resolvedAt:status==='OPEN'?null:'Just now'}:x)})),activity:[audit(`Document finding ${status.toLowerCase()}`,status==='RESOLVED'?'pass':'change'),...f.activity]})); },[mutate,remoteDocumentAction]);
  const confirmDocumentFindingBlocker = useCallback(async(finalizationId,findingId)=>{ if(REMOTE_MODE)return remoteDocumentAction(finalizationId,'confirm_finding_blocker',{findingId}); mutate(finalizationId,(f)=>({...f,artifacts:(f.artifacts||[]).map((a)=>({...a,documentFindings:(a.documentFindings||[]).map((x)=>x.id===findingId?{...x,severity:'BLOCKER',source:'HUMAN',status:'OPEN',resolutionNote:'Human reviewer confirmed this finding should block completion.'}:x)})),activity:[audit('Document finding confirmed as blocker','change'),...f.activity]})); },[mutate,remoteDocumentAction]);
  const reanalyzeDocument = useCallback(async(finalizationId,artifactId)=>{ if(REMOTE_MODE)return remoteDocumentAction(finalizationId,'reanalyze_artifact',{artifactId}); mutate(finalizationId,(f)=>({...f,artifacts:(f.artifacts||[]).map((a)=>a.id===artifactId?{...a,documentAnalysisStatus:'COMPLETE'}:a),activity:[audit('Document analysis re-run','change','Finalize'),...f.activity]})); },[mutate,remoteDocumentAction]);
  const reanalyzeDocumentPackage = useCallback(async(finalizationId)=>{ if(REMOTE_MODE)return remoteDocumentAction(finalizationId,'reanalyze_package'); mutate(finalizationId,(f)=>({...f,artifacts:(f.artifacts||[]).map((a)=>({...a,documentAnalysisStatus:a.parserStatus==='COMPLETE'?'COMPLETE':a.documentAnalysisStatus})),activity:[audit('Document package analysis re-run','change','Finalize'),...f.activity]})); },[mutate,remoteDocumentAction]);
  const setDocumentType = useCallback(async(finalizationId,artifactId,documentType)=>{ if(REMOTE_MODE)return remoteDocumentAction(finalizationId,'set_document_type',{artifactId,documentType}); mutate(finalizationId,(f)=>({...f,artifacts:(f.artifacts||[]).map((a)=>a.id===artifactId?{...a,documentType,documentTypeOverride:documentType,documentProfile:a.documentProfile?{...a.documentProfile,documentType}:a.documentProfile}:a),activity:[audit(`Document type set to ${documentType}`),...f.activity]})); },[mutate,remoteDocumentAction]);
  ...
  const bindIntegration = useCallback(async(finalizationId,payload)=>{
    if(REMOTE_MODE){const response=await fetch('/api/integrations/bindings',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({finalizationId,...payload})});const data=await response.json();if(!response.ok)throw new Error(data.detail||data.error||'Could not create integration binding');if(data.finalization)replaceOne(data.finalization);return data.binding;}
    mutate(finalizationId,(f)=>{const connection=(f.integrationConnections||[]).find((c)=>c.id===payload.connectionId);const req=f.requirements.find((r)=>r.id===payload.requirementId);const binding={id:`bind_${Math.random().toString(36).slice(2,8)}`,connectionId:payload.connectionId,requirementId:payload.requirementId,requirementTitle:req?.title||null,signalKey:payload.signalKey,expectedState:payload.expectedState,matcher:payload.matcher||{},required:payload.required!==false,status:'waiting',lastObservedState:null,lastObservedAt:null};return{...f,integrationBindings:[...(f.integrationBindings||[]),binding],requirements:f.requirements.map((r)=>r.id===payload.requirementId?{...r,type:'integration',status:'open',evidence:`Waiting for ${connection?.displayName||'external'} evidence`}:r),activity:[audit(`Integration binding created: ${payload.signalKey}`,'change','Finalize'),...f.activity]};});
  },[mutate,replaceOne]);

  const addDemoIntegrationConnection = useCallback((finalizationId,payload)=>{
    let result=[]; mutate(finalizationId,(f)=>{const connection={id:`int_${Math.random().toString(36).slice(2,8)}`,provider:payload.provider,displayName:payload.displayName||payload.provider,status:'configured',authMode:'environment',config:payload.config||{},lastSyncedAt:null};result=[...(f.integrationConnections||[]),connection];return{...f,integrationConnections:result,activity:[audit(`Integration configured: ${connection.displayName}`),...f.activity]};}); return result;
  },[mutate]);

  const simulateIntegrationEvidence = useCallback((finalizationId,provider,state)=>{
    mutate(finalizationId,(f)=>{const candidates=(f.integrationBindings||[]).filter((b)=>{const c=(f.integrationConnections||[]).find((x)=>x.id===b.connectionId);return c?.provider===provider;});const binding=candidates[0];if(!binding)return f;const passed=String(state).toLowerCase()===String(binding.expectedState).toLowerCase();const connection=(f.integrationConnections||[]).find((c)=>c.id===binding.connectionId);const summary=`${connection?.displayName||provider}: ${binding.signalKey} observed “${state}”${passed?' — expected state satisfied':` — expected “${binding.expectedState}”`}`;const now='Just now';const evidence={id:`ev_${Math.random().toString(36).slice(2,8)}`,provider,status:passed?'PASS':'FAIL',summary,signalKey:binding.signalKey,observedAt:now,reference:provider==='github'?'sha demo123':provider==='vercel'?'dpl_demo':provider==='stripe'?'pi_demo':null};return{...f,state:!passed&&f.state==='FINALIZED'?'REOPENED':f.state,integrationBindings:(f.integrationBindings||[]).map((b)=>b.id===binding.id?{...b,status:passed?'satisfied':'failed',lastObservedState:state,lastObservedAt:now}:b),externalEvidence:[evidence,...(f.externalEvidence||[])],requirements:f.requirements.map((r)=>r.id===binding.requirementId?{...r,status:passed?'passed':'open',evidence:summary,lastChecked:now}:r),approval:!passed&&f.state==='FINALIZED'?{...f.approval,status:'superseded',approvedAt:null}:f.approval,record:!passed&&f.state==='FINALIZED'?null:f.record,activity:[audit(summary,passed?'pass':'change','Finalize Integration Engine'),...f.activity]};});
  },[mutate]);
  ...
  return useMemo(()=>({workspace,finalizations,templates:PHASE2_TEMPLATES,hydrated,error,remoteMode:REMOTE_MODE,refresh,getById,getByToken,addParticipant,setRequirementStatus,recheckRequirement,addRequirement,addComment,addAnnotation,resolveAnnotation,addFileRequest,completeFileRequest,addSecureRequest,submitSecureRequest,destroySecureRequest,createPaymentGate,addPrivacyItem,setPaymentStatus,setPrivacyItemStatus,sendReminder,setDocumentFindingStatus,confirmDocumentFindingBlocker,reanalyzeDocument,reanalyzeDocumentPackage,setDocumentType,bindIntegration,addDemoIntegrationConnection,simulateIntegrationEvidence,requestApproval,approve,finalize,createFinalization,createGuestLink,updateBranding,updateReviewUrl,bumpDemoVersion,resetDemo}),[workspace,finalizations,hydrated,error,refresh,getById,getByToken,addParticipant,setRequirementStatus,recheckRequirement,addRequirement,addComment,addAnnotation,resolveAnnotation,addFileRequest,completeFileRequest,addSecureRequest,submitSecureRequest,destroySecureRequest,createPaymentGate,addPrivacyItem,setPaymentStatus,setPrivacyItemStatus,sendReminder,setDocumentFindingStatus,confirmDocumentFindingBlocker,reanalyzeDocument,reanalyzeDocumentPackage,setDocumentType,bindIntegration,addDemoIntegrationConnection,simulateIntegrationEvidence,requestApproval,approve,finalize,createFinalization,createGuestLink,updateBranding,updateReviewUrl,bumpDemoVersion,resetDemo]);
}
