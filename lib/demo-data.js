...
const BASE_FINALIZATIONS = [
  ...
  },
  {
    ...
    comments: [],
    reviewUrl: null, versions: [{id:'ver_1',versionNumber:1,artifactId:null,reason:'room_created',createdAt:'Just now'}], templateKey: null, handoffStatus: 'NOT_STARTED', privacyCloseoutStatus: 'OPEN', annotations: [], fileRequests: [], secureRequests: [], paymentGates: [], privacyItems: [], reminders: [], integrationConnections: [], integrationBindings: [], externalEvidence: [],
...
const PHASE4_DOCUMENT_DEMO = {
  fin_acme: {
    art_acme_1: { documentAnalysisStatus:'COMPLETE', documentType:'REPORT', documentScore:88, documentProfile:{id:'dp_acme_1',documentType:'REPORT',specKey:'report-completeness',title:'Acme Launch Handoff Report',language:'en',score:88,metrics:{characters:18420,words:3128,headings:12,references:2,placeholders:0},structure:{headings:['EXECUTIVE SUMMARY','LAUNCH CONDITIONS','HANDOFF','APPENDIX A']},entities:{legalEntities:['Acme & Co.'],dates:['August 20, 2026'],currencies:['CAD']},analyzedAt:'Today, 2:52 AM'},documentFindings:[{id:'df_acme_1',ruleKey:'sources_present',severity:'WARNING',source:'DETERMINISTIC',title:'Sources/references are present when claims require them',detail:'The Report Completeness standard could not verify a references section.',evidence:{snippet:'Performance and accessibility results are summarized below.'},status:'OPEN',createdAt:'Today, 2:52 AM'}],documentReferences:[{id:'dr_acme_1',type:'Appendix',label:'A',rawText:'Appendix A',present:true},{id:'dr_acme_2',type:'Attachment',label:'2',rawText:'Attachment 2',present:true}] },
    art_acme_2: { documentAnalysisStatus:'COMPLETE', documentType:'GENERIC', documentScore:100, documentProfile:{id:'dp_acme_2',documentType:'GENERIC',specKey:'generic-document',title:'launch-checklist.txt',language:'en',score:100,metrics:{characters:9420,words:1470,headings:8,references:0,placeholders:0},structure:{headings:['PRE-LAUNCH','DNS','FORMS','MOBILE QA']},entities:{legalEntities:[],dates:['August 20, 2026'],currencies:[]},analyzedAt:'Today, 2:53 AM'},documentFindings:[],documentReferences:[] }
  },
  fin_vendor: {
    art_vendor_1: { documentAnalysisStatus:'COMPLETE', documentType:'CONTRACT', documentScore:57, documentProfile:{id:'dp_vendor_1',documentType:'CONTRACT',specKey:'contract-completeness',title:'Vendor Services Agreement',language:'en',score:57,metrics:{characters:22140,words:3860,headings:18,references:2,placeholders:2},structure:{headings:['SERVICES','FEES','TERM','TERMINATION','SIGNATURES']},entities:{legalEntities:['Riverbend Supply Ltd.','Northstar Studio Inc.'],dates:['August 22, 2026'],currencies:['CAD']},analyzedAt:'Yesterday'},documentFindings:[
      {id:'df_vendor_1',ruleKey:'references_present',severity:'BLOCKER',source:'DETERMINISTIC',title:'1 referenced attachment is missing',detail:'The agreement references Schedule B, but the current Finalization package does not contain a matching file.',evidence:{snippet:'Fees and service levels are set out in Schedule B.',references:[{kind:'Schedule',label:'B',raw:'Schedule B'}]},status:'OPEN',createdAt:'Yesterday'},
      {id:'df_vendor_2',ruleKey:'no_placeholders',severity:'BLOCKER',source:'DETERMINISTIC',title:'2 unresolved placeholders detected',detail:'Replace unfinished template fields before finalization.',evidence:{snippet:'Effective Date: ________',count:2},status:'OPEN',createdAt:'Yesterday'},
      {id:'df_vendor_3',ruleKey:'signature_ready',severity:'BLOCKER',source:'DETERMINISTIC',title:'Signature section appears complete',detail:'The Contract Completeness standard found blank signature/name/date fields.',evidence:{snippet:'Signature: ________  Name: ________'},status:'OPEN',createdAt:'Yesterday'},
      {id:'df_vendor_4',ruleKey:'governing_law',severity:'WARNING',source:'DETERMINISTIC',title:'Governing law is stated',detail:'The Contract Completeness standard could not verify a governing-law clause.',evidence:{},status:'OPEN',createdAt:'Yesterday'}
    ],documentReferences:[{id:'dr_vendor_1',type:'Schedule',label:'A',rawText:'Schedule A',present:true},{id:'dr_vendor_2',type:'Schedule',label:'B',rawText:'Schedule B',present:false}] }
  },
};
...
const PHASE5_DEMO = {
  fin_acme: {
    integrationConnections: [
      {id:'int_git',provider:'github',displayName:'GitHub · acme-web',status:'connected',authMode:'environment',config:{repository:'northstar/acme-web'},lastSyncedAt:'Today, 2:41 AM'},
      {id:'int_vercel',provider:'vercel',displayName:'Vercel · acme-production',status:'connected',authMode:'environment',config:{projectId:'prj_acme'},lastSyncedAt:'Today, 2:43 AM'},
      {id:'int_stripe',provider:'stripe',displayName:'Stripe · Northstar',status:'connected',authMode:'webhook',config:{},lastSyncedAt:'Yesterday, 5:02 PM'},
    ],
    integrationBindings: [
      {id:'bind_mobile',connectionId:'int_vercel',requirementId:'req_mobile',requirementTitle:'Mobile navigation re-check',signalKey:'vercel.deployment',expectedState:'ready',matcher:{projectId:'prj_acme'},status:'waiting',lastObservedState:'building',lastObservedAt:'Today, 2:43 AM'},
      {id:'bind_invoice',connectionId:'int_stripe',requirementId:'req_invoice',requirementTitle:'Final invoice paid',signalKey:'stripe.payment',expectedState:'paid',matcher:{},status:'waiting',lastObservedState:'requires_payment_method',lastObservedAt:'Yesterday, 5:02 PM'},
    ],
    externalEvidence: [
      {id:'ev_git_1',provider:'github',status:'PASS',summary:'GitHub · acme-web: required workflow completed successfully',signalKey:'github.workflow_run',observedAt:'Today, 2:41 AM',reference:'sha 0e7c2d1'},
      {id:'ev_vercel_1',provider:'vercel',status:'INFO',summary:'Vercel deployment is still building',signalKey:'vercel.deployment',observedAt:'Today, 2:43 AM',reference:'dpl_acme_28'},
    ],
  },
  fin_vendor: {
    integrationConnections: [{id:'int_esign',provider:'docusign',displayName:'E-signature · vendor agreements',status:'configured',authMode:'webhook',config:{},lastSyncedAt:null}],
    integrationBindings: [{id:'bind_sign',connectionId:'int_esign',requirementId:'v3',requirementTitle:'Vendor signature received',signalKey:'esign.envelope',expectedState:'completed',matcher:{},status:'waiting',lastObservedState:null,lastObservedAt:null}],
    externalEvidence: [],
  },
  fin_brand: {integrationConnections:[],integrationBindings:[],externalEvidence:[]},
};
...
export const INITIAL_FINALIZATIONS = BASE_FINALIZATIONS.map((f) => ({
  ...
  artifacts: (PHASE2_DEMO[f.id]?.artifacts || []).map((a) => ({...a,...(PHASE4_DOCUMENT_DEMO[f.id]?.[a.id] || {})})),
}));
