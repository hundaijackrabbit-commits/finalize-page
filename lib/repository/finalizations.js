...
export async function readinessFromDb(ctx, id) {
  const [reqRes, aprRes, fileRes, secureRes, payRes, privacyRes, finRes, annRes, docRes, artifactRes] = await Promise.all([
  ...
  for (const result of [reqRes,fileRes,secureRes,payRes,privacyRes,finRes,annRes,docRes,artifactRes]) if (result.error) throw result.error;
